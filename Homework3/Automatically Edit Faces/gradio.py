...
def detect_face_landmarks(image):
    ...
    landmarks = fa.get_landmarks(np_image)
    if landmarks is None:
        print("未检测到人脸关键点")
        return None
    return landmarks[0]  # 返回检测到的第一个人脸的关键点

def apply_face_editing(landmarks, operation):
    """
    根据用户选择的操作修改人脸关键点
    :param landmarks: 人脸关键点数组
    :param operation: 操作类型 ('close_eyes', 'close_mouth', 'enlarge_eyes')
    """
    if landmarks is None:
        print("未检测到人脸关键点，无法操作")
        return None

    # 关键点索引：参考 https://github.com/1adrianb/face-alignment
    EYE_L_IDX = [36, 37, 38, 39, 40, 41]  # 左眼
    EYE_R_IDX = [42, 43, 44, 45, 46, 47]  # 右眼
    MOUTH_IDX = list(range(48, 68))       # 嘴巴

    if operation == 'close_eyes':
        for idx in EYE_L_IDX + EYE_R_IDX:
            landmarks[idx, 1] += 2  # 向下移动眼睛点
    elif operation == 'close_mouth':
        for idx in MOUTH_IDX:
            if idx < 60:  # 外嘴唇
                landmarks[idx, 1] -= 3  # 向上移动
            else:  # 内嘴唇
                landmarks[idx, 1] += 3  # 向下移动
    elif operation == 'enlarge_eyes':
        for eye in [EYE_L_IDX, EYE_R_IDX]:
            center = landmarks[eye].mean(axis=0)
            for idx in eye:
                landmarks[idx] = center + (landmarks[idx] - center) * 1.2  # 放大眼睛点
    else:
        print("未知操作")
    return landmarks
...
with gr.Blocks() as app:
    ...
    with gr.Row():
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3):
                ...
                with gr.Row():
                    with gr.Column(scale=1, min_width=10):
                        gr.Markdown(value='close_eyes', show_label=False)
                    with gr.Column(scale=4, min_width=10):
                        form_close_eyes_btn= gr.Button('close_eyes')
                with gr.Row():
                    with gr.Column(scale=1, min_width=10):
                        gr.Markdown(value='close_mouth', show_label=False)
                    with gr.Column(scale=4, min_width=10):
                        form_close_mouth_btn= gr.Button('close_mouth')
                with gr.Row():
                    with gr.Column(scale=1, min_width=10):
                        gr.Markdown(value='enlarge_eyes', show_label=False)
                    with gr.Column(scale=4, min_width=10):
                        form_enlarge_eyes_btn= gr.Button('enlarge_eyes')
                with gr.Row():
                    with gr.Column(scale=1, min_width=10):
                        gr.Markdown(value='Face_slimming', show_label=False)
                    with gr.Column(scale=4, min_width=10):
                        form_Face_slimming_btn= gr.Button('Face_slimming')
                with gr.Row():
                    with gr.Column(scale=1, min_width=10):
                        gr.Markdown(value='smile', show_label=False)
                    with gr.Column(scale=4, min_width=10):
                        form_smile_btn= gr.Button('smile')
    ...
    def on_click_close_eyes(global_state):
        ...
        landmarks = fa.get_landmarks(np.array(image_raw))
        ...
        points = global_state["points"]
        points[0] = {'start': landmarks_int[0][37], 'target': landmarks_int[0][41]}
        points[1] = {'start': landmarks_int[0][44], 'target': landmarks_int[0][46]}
        points[2] = {'start': landmarks_int[0][38], 'target': landmarks_int[0][40]}
        points[3] = {'start': landmarks_int[0][43], 'target': landmarks_int[0][47]}
        image_draw = update_image_draw(
            image_raw,
            global_state['points'],
            global_state['mask'],
    ...
    form_close_eyes_btn.click(
        on_click_close_eyes,
        inputs=[global_state],
        outputs=[global_state,form_image],
    )
    
    def on_click_close_mouth(global_state):
        image_raw = global_state['images']['image_raw']
        fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, flip_input=False)
        landmarks = fa.get_landmarks(np.array(image_raw))
        # 将 landmarks 转换为 numpy 数组
        ...
        landmarks_int = landmarks.astype(int)        
        points = global_state["points"]
        points[0] = {'start': landmarks_int[0][61], 'target': landmarks_int[0][67]}
        points[1] = {'start': landmarks_int[0][63], 'target': landmarks_int[0][65]}
        #points[2] = {'start': landmarks_int[0][38], 'target': landmarks_int[0][40]}
        #points[3] = {'start': landmarks_int[0][43], 'target': landmarks_int[0][47]}
        image_draw = update_image_draw(
            image_raw,
    ...
    form_close_mouth_btn.click(
        on_click_close_mouth,
        inputs=[global_state],
        outputs=[global_state,form_image],
    )
    
    def on_click_enlarge_eyes(global_state):
        image_raw = global_state['images']['image_raw']
        fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, flip_input=False)
        landmarks = fa.get_landmarks(np.array(image_raw))
        # 将 landmarks 转换为 numpy 数组
        ...
        landmarks_int = landmarks.astype(int)        
        points = global_state["points"]
        points[0] = {'start': landmarks_int[0][37], 'target': (landmarks_int[0][37][0],landmarks_int[0][37][1]-20)}
        points[1] = {'start': landmarks_int[0][38], 'target': (landmarks_int[0][38][0],landmarks_int[0][38][1]-20)}
        points[2] = {'start': landmarks_int[0][44], 'target': (landmarks_int[0][44][0],landmarks_int[0][44][1]-20)}
        points[3] = {'start': landmarks_int[0][43], 'target': (landmarks_int[0][43][0],landmarks_int[0][43][1]-20)}
        image_draw = update_image_draw(
            image_raw,
            global_state['points'],
            global_state['mask'],
    ...
    form_enlarge_eyes_btn.click(
        on_click_enlarge_eyes,
        inputs=[global_state],
        outputs=[global_state,form_image],
    )
    
    def on_click_Face_slimming(global_state):
        image_raw = global_state['images']['image_raw']
        fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, flip_input=False)
        landmarks = fa.get_landmarks(np.array(image_raw))
        # 将 landmarks 转换为 numpy 数组
        ...
        landmarks_int = landmarks.astype(int)        
        points = global_state["points"]
        points[0] = {'start': landmarks_int[0][2], 'target': (landmarks_int[0][2][0]+30,landmarks_int[0][2][1])}
        points[1] = {'start': landmarks_int[0][3], 'target': (landmarks_int[0][3][0]+30,landmarks_int[0][3][1])}
        points[2] = {'start': landmarks_int[0][4], 'target': (landmarks_int[0][4][0]+30,landmarks_int[0][4][1])}
        points[3] = {'start': landmarks_int[0][5], 'target': (landmarks_int[0][5][0]+30,landmarks_int[0][5][1])}
        points[4] = {'start': landmarks_int[0][6], 'target': (landmarks_int[0][6][0]+30,landmarks_int[0][6][1])}
        points[5] = {'start': landmarks_int[0][10], 'target': (landmarks_int[0][10][0]-30,landmarks_int[0][10][1])}
        points[6] = {'start': landmarks_int[0][11], 'target': (landmarks_int[0][11][0]-30,landmarks_int[0][11][1])}
        points[7] = {'start': landmarks_int[0][12], 'target': (landmarks_int[0][12][0]-30,landmarks_int[0][12][1])}
        points[8] = {'start': landmarks_int[0][13], 'target': (landmarks_int[0][13][0]-30,landmarks_int[0][13][1])}
        points[9] = {'start': landmarks_int[0][14], 'target': (landmarks_int[0][14][0]-30,landmarks_int[0][14][1])}
        image_draw = update_image_draw(
            image_raw,
            global_state['points'],
            global_state['mask'],
    ...
    form_Face_slimming_btn.click(
        on_click_Face_slimming,
        inputs=[global_state],
        outputs=[global_state,form_image],
    )
    ...
    def on_click_smile(global_state):
        ...
        landmarks_int = landmarks.astype(int)        
        points = global_state["points"]
        points[0] = {'start': landmarks_int[0][48], 'target': (landmarks_int[0][48][0]-30,landmarks_int[0][48][1]-30)}
        points[1] = {'start': landmarks_int[0][54], 'target': (landmarks_int[0][54][0]+30,landmarks_int[0][54][1]-30)}
        image_draw = update_image_draw(
            image_raw,
            global_state['points'],
            global_state['mask'],

...
def create_mask_from_points(points, img_h, img_w):
    ...
    pil_mask = Image.new('L', (img_w, img_h), 0)  # 'L' mode for grayscale

    # Create a draw object
    draw = ImageDraw.Draw(pil_mask)

    # Ensure points are in the correct format
    points_list = [tuple(point) for point in points]  # Convert to list of tuples

    # Draw the polygon on the mask
    draw.polygon(points_list, outline=0, fill=255)

    # Convert the PIL image back to a numpy array
    mask = np.array(pil_mask)

    # Scale mask: 0 indicates outside the Polygon, 255 indicates inside the Polygon
    return mask
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    laplacian_filter = torch.tensor([[0, 1, 0],
                                      [1, -4, 1],
                                      [0, 1, 0]], dtype=torch.float32, device=foreground_img.device).unsqueeze(0).repeat(3, 1, 1).unsqueeze(0)
    
    # Apply the Laplacian filter to the foreground and blended images
    fg_laplacian = F.conv2d(foreground_img, laplacian_filter, padding=1)
    blended_laplacian = F.conv2d(blended_img, laplacian_filter, padding=1)

    # Mask the results with the foreground and background masks

    # Compute the Laplacian loss
    loss = F.mse_loss(fg_laplacian[foreground_mask.bool()], blended_laplacian[background_mask.bool()])
